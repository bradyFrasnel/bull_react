// Le secrétariat a les mêmes droits que l'admin pour la saisie des notes.
// On réutilise le même composant avec le layout secrétariat.
...
import { AdminLayout as SecretariatLayout } from '../../components/AdminLayout';
...
export const SaisirNotes: React.FC = () => {
  ...
  const fetchEvaluations = async () => {
    try {
      const data = await evaluationService.getByEtudiantAndMatiere(selectedEtudiant, selectedMatiere);
      setEvaluations([{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(note) || note < 0 || note > 20) {
      setError('La note doit être comprise entre 0 et 20'); return;
    }
    if (noteForm.type === 'RATTRAPAGE') {
      const v = await evaluationService.validateRattrapage(selectedEtudiant, selectedMatiere);
      if (!v.autorise) { setError(v.raison || 'Rattrapage non autorisé'); return; }
    }
    try {
      ...
      if (existing) {
        ...
        const payload: CreateEvaluationForm = {
          utilisateurId: selectedEtudiant, matiereId: selectedMatiere,
          type: noteForm.type, note, saisiePar: '',
        };
  ...
  const statusIcon = (ev?: Evaluation) =>
    ev ? <CheckCircle className="w-5 h-5 text-green-500" /> : <XCircle className="w-5 h-5 text-red-400" />;
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    <BookOpen className="w-4 h-4 inline mr-1" />Matière
                  </label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    <Users className="w-4 h-4 inline mr-1" />Étudiant
                  </label>
                      ...
                      return (
                        <div key={type} className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            {statusIcon(ev)}
                            {ev && <span className="text-sm font-medium">{ev.note}/20</span>}
                          </div>
                        </div>
                      );
                    })}
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              {selectedMatiere && selectedEtudiant ? (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="number" min="0" max="20" step="0.01" value={noteForm.note}
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button type="submit" disabled={submitting || !noteForm.note}
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">Sélectionnez une matière et un étudiant pour commencer</p>
                </div>
              )}
            </div>
          </div>
